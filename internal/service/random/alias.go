...
// New Create a new alias object.
// For example,
//
//	var v = alias.New([]float32{8,10,2})
//
// creates an alias that returns 0 40% of the time, 1 50% of the time, and
// 2 10% of the time.
func New(prob []float32) (*Alias, error) {
...
func (al *Alias) Gen() uint32 {
	v, _ := rand.Int(rand.Reader, big.NewInt(math.MaxInt32))
	ri := uint32(v.Int64())
